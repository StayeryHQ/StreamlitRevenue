...
Drei Teile:
  ...
  - ``section(N, title)``        - Context Manager, immer rendert (mit Anchor).
  - ``lazy_section(N, title)``   - Click-to-load (mit Anchor). Verhindert dass
                                    schwere Charts auf jedem Rerun zum Browser
                                    geschickt werden.
...
def _anchor_id(num: int | str) -> str:
    """Stable anchor id for a section, safe for HTML."""
    s = str(num).replace(".", "-")
    return f"sec-{s}"


def _anchor_html(num: int | str) -> str:
    """Invisible <a id> anchor that scroll-jumps land on."""
